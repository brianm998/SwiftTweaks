...
internal enum TweakViewData {
	...
	case integer(value: Int, defaultValue: Int, min: Int?, max: Int?, stepSize: Int?, editStyle: NumericEditStyle)
	case int8(value: Int8, defaultValue: Int8, min: Int8?, max: Int8?, stepSize: Int8?, editStyle: NumericEditStyle)
	case int16(value: Int16, defaultValue: Int16, min: Int16?, max: Int16?, stepSize: Int16?, editStyle: NumericEditStyle)
	case int32(value: Int32, defaultValue: Int32, min: Int32?, max: Int32?, stepSize: Int32?, editStyle: NumericEditStyle)
	case int64(value: Int64, defaultValue: Int64, min: Int64?, max: Int64?, stepSize: Int64?, editStyle: NumericEditStyle)
	case uInt8(value: UInt8, defaultValue: UInt8, min: UInt8?, max: UInt8?, stepSize: UInt8?, editStyle: NumericEditStyle)
	case uInt16(value: UInt16, defaultValue: UInt16, min: UInt16?, max: UInt16?, stepSize: UInt16?, editStyle: NumericEditStyle)
	case uInt32(value: UInt32, defaultValue: UInt32, min: UInt32?, max: UInt32?, stepSize: UInt32?, editStyle: NumericEditStyle)
	case uInt64(value: UInt64, defaultValue: UInt64, min: UInt64?, max: UInt64?, stepSize: UInt64?, editStyle: NumericEditStyle)
	case float(value: CGFloat, defaultValue: CGFloat, min: CGFloat?, max: CGFloat?, stepSize: CGFloat?, editStyle: NumericEditStyle)
	case doubleTweak(value: Double, defaultValue: Double, min: Double?, max: Double?, stepSize: Double?, editStyle: NumericEditStyle)
	...
	init<T: TweakableType>(type: TweakViewDataType, value: T, defaultValue: T, minimum: T?, maximum: T?, stepSize: T?, options: [T]?, editStyle: NumericEditStyle = SwiftTweaks.defaultNumericEditingStyle) {
		switch type {
		case .boolean:
		...
		case .int8:
			...
			self = .int8(value: clippedValue, defaultValue: defaultValue as! Int8, min: minimum as? Int8, max: maximum as? Int8, stepSize: stepSize as? Int8, editStyle: editStyle)
		case .int16:
			let clippedValue = clip(value as! Int16, minimum as? Int16, maximum as? Int16)
			self = .int16(value: clippedValue, defaultValue: defaultValue as! Int16, min: minimum as? Int16, max: maximum as? Int16, stepSize: stepSize as? Int16, editStyle: editStyle)
		case .int32:
			let clippedValue = clip(value as! Int32, minimum as? Int32, maximum as? Int32)
			self = .int32(value: clippedValue, defaultValue: defaultValue as! Int32, min: minimum as? Int32, max: maximum as? Int32, stepSize: stepSize as? Int32, editStyle: editStyle)
		case .int64:
			let clippedValue = clip(value as! Int64, minimum as? Int64, maximum as? Int64)
			self = .int64(value: clippedValue, defaultValue: defaultValue as! Int64, min: minimum as? Int64, max: maximum as? Int64, stepSize: stepSize as? Int64, editStyle: editStyle)
		case .uInt8:
			let clippedValue = clip(value as! UInt8, minimum as? UInt8, maximum as? UInt8)
			self = .uInt8(value: clippedValue, defaultValue: defaultValue as! UInt8, min: minimum as? UInt8, max: maximum as? UInt8, stepSize: stepSize as? UInt8, editStyle: editStyle)
		case .uInt16:
			let clippedValue = clip(value as! UInt16, minimum as? UInt16, maximum as? UInt16)
			self = .uInt16(value: clippedValue, defaultValue: defaultValue as! UInt16, min: minimum as? UInt16, max: maximum as? UInt16, stepSize: stepSize as? UInt16, editStyle: editStyle)
		case .uInt32:
			let clippedValue = clip(value as! UInt32, minimum as? UInt32, maximum as? UInt32)
			self = .uInt32(value: clippedValue, defaultValue: defaultValue as! UInt32, min: minimum as? UInt32, max: maximum as? UInt32, stepSize: stepSize as? UInt32, editStyle: editStyle)
		case .uInt64:
			let clippedValue = clip(value as! UInt64, minimum as? UInt64, maximum as? UInt64)
			self = .uInt64(value: clippedValue, defaultValue: defaultValue as! UInt64, min: minimum as? UInt64, max: maximum as? UInt64, stepSize: stepSize as? UInt64, editStyle: editStyle)
		case .integer:
			let clippedValue = clip(value as! Int, minimum as? Int, maximum as? Int)
			self = .integer(value: clippedValue, defaultValue: defaultValue as! Int, min: minimum as? Int, max: maximum as? Int, stepSize: stepSize as? Int, editStyle: editStyle)
		case .cgFloat:
			let clippedValue = clip(value as! CGFloat, minimum as? CGFloat, maximum as? CGFloat)
			self = .float(value: clippedValue, defaultValue: defaultValue as! CGFloat, min: minimum as? CGFloat, max: maximum as? CGFloat, stepSize: stepSize as? CGFloat, editStyle: editStyle)
		case .double:
			let clippedValue = clip(value as! Double, minimum as? Double, maximum as? Double)
			self = .doubleTweak(value: clippedValue, defaultValue: defaultValue as! Double, min: minimum as? Double, max: maximum as? Double, stepSize: stepSize as? Double, editStyle: editStyle)
		case .string:
	...
	var value: TweakableType {
		...
		case let .boolean(value: boolValue, defaultValue: _):
			return boolValue
		case let .integer(value: intValue, _, _, _, _, _):
			return intValue
		case let .int8(value: intValue, _, _, _, _, _):
			return intValue
		case let .int16(value: intValue, _, _, _, _, _):
			return intValue
		case let .int32(value: intValue, _, _, _, _, _):
			return intValue
		case let .int64(value: intValue, _, _, _, _, _):
			return intValue
		case let .uInt8(value: intValue, _, _, _, _, _):
			return intValue
		case let .uInt16(value: intValue, _, _, _, _, _):
			return intValue
		case let .uInt32(value: intValue, _, _, _, _, _):
			return intValue
		case let .uInt64(value: intValue, _, _, _, _, _):
			return intValue
	...
	var stringRepresentation: (String, Bool) {
		...
		case let .integer(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "Int(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .int8(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "Int8(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .int16(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "Int16(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .int32(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "Int32(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .int64(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "Int64(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .uInt8(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "UInt8(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .uInt16(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "UInt16(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .uInt32(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "UInt32(\(value))"
			differsFromDefault = (value != defaultValue)
		case let .uInt64(value: value, defaultValue: defaultValue, _, _, _, _):
			string = "UInt64(\(value))"
			differsFromDefault = (value != defaultValue)
	...
	/// Represents the configuration for a UIStepper's and UISliders' min/max/stepSize values
	internal typealias Bounds = (min: Double, max: Double, stepSize: Double)
	...
	var bounds: Bounds? {
		...
		case let .integer(intValue, intDefaultValue, intMin, intMax, intStep, _):
		...
		case let .int8(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
		...
		case let .int16(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
		...
		case let .int32(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
		...
		case let .int64(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
		...
		case let .uInt8(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
		...
		case let .uInt16(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
		...
		case let .uInt32(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
		...
		case let .uInt64(intValue, intDefaultValue, intMin, intMax, intStep, _):
			currentValue = Double(intValue)
			defaultValue = Double(intDefaultValue)
			minimum = intMin.map(Double.init)
			maximum = intMax.map(Double.init)
